...
//initialize struct and array 2d and necesarry variables
struct jag{
    char index[20];
};
...
int size(char *p) //return size of a string array
{
    int k = 0;
    int count = 0;
    while (*(p + k) != '\0')
...
void input_NFA() //this function is to take input of nfa
{
    int state, symbol, a;
    NFA[0][0].index[0] = '-'; //initialize very first intes first index with '-'
    printf("**Total number of states: ");
    scanf("%d",&state);
    getchar();
    NFA_r = state;
    printf("**Enter states with a space: ");
    for(int i = 1; i <= NFA_r; i++) //taking input of states and initilalizing into the nfa array and states array
    {
        char x;
        scanf("%c",&x);
        states[i-1] = x;
    ...
    for(int i = 1; i <= NFA_c; i++) //taking input of sybol and initilalizing into the nfa array, dfa array and sybols array
    {
        char x;
        scanf("%c",&x);
        NFA[0][i].index[0] = x;
    ...
    for(int i = 1 ; i <= NFA_r; i++) //initializing the first index of each nfa with P
    {
    ...
    for(int i = 1 ; i <= NFA_r; i++) //taking input of transitions with each symmbol and writing it down in the file 
    {
        for(int l = 1; l <= NFA_c; l++)
        {
            ...
            if((string[0] >= 60 && string[0] <= 90) ||
               (string[0] >= 97 && string[0] <= 122)) //alfo if each symmbol are with in A-Z or a-z or 0-9 we copy int into nfa array's index
            {
...
void input_eNFA() //this function is to take input of enfa and initialize along
{
    int state, symbol, a;
    eNFA[0][0].index[0] = '-';
    printf("**Total number of states: ");
    ...
    eNFA_r = state;
    printf("**Enter states with a space: ");
    for(int i = 1; i <= eNFA_r; i++) //enter states into states array and enafa array
    {
        char x;
        scanf("%c",&x);
        states[i-1] = x;
    ...
    eNFA_c = symbol;
    printf("**Enter symbols with a space: ");
    for(int i = 1; i <= eNFA_c; i++) //enter symbol into symbols array and enfa array
    {
        char x;
        scanf("%c",&x);
        eNFA[0][i].index[0] = x;
    ...
    for(int i = 1 ; i <= eNFA_r; i++) //for each items in enfa we enter p as initially in the first of index
    {
    ...
    for(int i = 1 ; i <= eNFA_r; i++) // taking input of each trasitions with each symbol and writing it down in the text file
    {
        for(int l = 1; l <= eNFA_c; l++)
        {
            ...
            if((string[0] >= 60 && string[0] <= 90) ||
               (string[0] >= 97 && string[0] <= 122)||
                (string[0] >= '0' && string[0] <= '9')) //alfo if each symmbol are with in A-Z or a-z or 0-9 we copy int into enfa array's index
            {
...
void apend(char a, int n, int r){ //append new char into result
	result[n][r] = a;
}
void apend1(char a, int r){ //append new char into  states
	states2[r] = a;
}
void apend_dfa(char a, int r) //append new char into backup
{
    backup[r] = a;
}


char* removeDuplicates(char arr[], int n) //remove duplicates char in a string
{
    int j = 0;
...
void eNFA_to_NFA()
{
    input_eNFA(); //take input of enfa
    printf("\n     \"\"Inputed epsilon-NFA\"\"");
    show_eNFA(); //print enfa
    s = size(&states[0]);
    s1 = size(&symbols[0]);
	for(int k = 1; k <= s; k++){ //
...
int main()
{
    FILE *fptr; //take transition input from the file
    ...
    //If there are any epsilon in the transition the we first convert enfa to nfa first and if not then nfa to dfa
    if(ch == 'Y' || ch == 'y')
    {
        access = true;
        eNFA_to_NFA();
